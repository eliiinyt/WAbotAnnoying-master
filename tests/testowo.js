...
async function generateCharacterCard(player_id, index = 0) {
  ...
  // maybe es útil luego console.log(player.toGOOD())
  const charactersPreview = player.charactersPreview;
  let character_details = player;
  let stats = character_details.characters[0].stats;
  let costume = character_details.characters[0].costume;
  skillTalent = character_details.characters[0].skillLevels;

  const character_card = {
    level: character_details.characters[0].level,
    costumes: {
      splashImage: {
    ...
    characterData: {
      icon: {
        name: character_details.characters[0].characterData.icon.name,
        url: character_details.characters[0].characterData.icon.url,
        imageBaseUrl:
          character_details.characters[0].characterData.icon.imageBaseUrl,
      },
      nameCard: {
        name: character_details.characters[0].characterData.nameCard.pictures[1]
          .name,
        url: character_details.characters[0].characterData.nameCard.pictures[1]
          .url,
      },
      id: character_details.characters[0].characterData.id,
      description: character_details.characters[0].characterData.description,
      name: character_details.characters[0].characterData.name.get("es"),
      gender: character_details.characters[0].characterData.gender,
      weaponType: character_details.characters[0].characterData.weaponType,
      _nameId: character_details.characters[0].characterData._nameId,
      isArchon: character_details.characters[0].characterData.isArchon,
      isTraveler: character_details.characters[0].characterData.isTraveler,
      stars: character_details.characters[0].characterData.stars,
      rarity: character_details.characters[0].characterData.rarity,
      constellations:
        character_details.characters[0].characterData.constellations,
      elementalSkill:
        character_details.characters[0].characterData.elementalSkill,
      elementalBurst:
        character_details.characters[0].characterData.elementalBurst,
      skills: character_details.characters[0].characterData.skills,
      normalAttack: character_details.characters[0].characterData.normalAttack,
      PassiveTalents:
        character_details.characters[0].characterData.passiveTalents,
      element: character_details.characters[0].characterData.element,
      bodyType: character_details.characters[0].characterData.bodyType,
      releasedAt: character_details.characters[0].characterData.releasedAt,
      skillDepotId: character_details.characters[0].characterData.skillDepotId,
    },
    weapon: {
      level: character_details.characters[0].weapon.level,
      refinementRank: character_details.characters[0].weapon.refinementRank,
      weaponData: {
        icon: {
          name: character_details.characters[0].weapon.weaponData.icon.name,
          url: character_details.characters[0].weapon.weaponData.icon.url
        },
        name: character_details.characters[0].weapon.weaponData.name.get("es"),
        stars: character_details.characters[0].weapon.weaponData.stars
      },
      weaponStats: character_details.characters[0].weapon.weaponStats.filter(stat => stat.value !== 0)
    },
    characterStats: {
      ...
      // elementalDamageBonus: (stats.matchedElementDamage.value * 100).toFixed(1),
    ...
    artifacts: character_details.characters[0].artifacts.map((artifact) => {
      ...
      return {
        ...
        mainstat: {
          ...
          value: mainstat.isPercent
            ? (mainstat.value * 100).toFixed(1)
            : mainstat.value.toFixed(0),
        },
        substats: {
          total: substats.total.map((total) => ({
            total: {
              ...
              value: total.isPercent
                ? (total.value * 100).toFixed(1)
                : total.value.toFixed(0),
          ...
          split: substats.split.map((split) => ({
            split: {
              ...
              value: split.isPercent
                ? (split.value * 100).toFixed(1)
                : split.value.toFixed(0),
    ...
    unlockedConstellations:
      character_details.characters[0].unlockedConstellations,
  };
  ...
  const drawArtifacts = async (ctx, character_card, x, y) => {
  ...
  async function drawConstellation(ctx, character_card) {
    characterData = character_card.characterData;
    unlockedConstellations = character_card.unlockedConstellations;
    //console.log(character_card.characterData.constellations);
    ...
    for (let i = 0; i < constellations.length; i++) {
      ...
      console.log(constellation.icon.url);
  ...
  return generateCharacterCard(character_card);

  console.log(character_card);

  let cha = charactersPreview.map((character) => {
  ...
  async function generateCharacterCard(character_card) {
    ...
    const stats = character_card.characterStats;
    ...
    const out = fs.createWriteStream(
      path.join(__dirname, "character_detail.png")
    );
    ...
    out.on("finish", () => {
      console.log(
        "Character Showcase generado y guardado como character_detail.png"
      );
    });
  }
  ...
  async function generateCharacterShowcase() {
    ...
    for (let i = 0; i < cha.length; i++) {
      ...
      //ctx.beginPath(); podría ser útil luego
      //ctx.arc(x + 40, y + 40, 15, 0, Math.PI * 2);
      //ctx.fill();
      ...
      function drawText(ctx, text, x, y) {
        ...
        ctx.fillText(ctx, text, centerX, y);
      }
      ...
      //ctx.fillStyle = '#333';
      //ctx.font = '16px Arial';
      //ctx.fillText(`Lv. ${level}`, x, y + 170);
      //ctx.fillText(_data._nameId, x, y + 190);
    }
    const out = fs.createWriteStream(
      path.join(__dirname, "character-showcase.png")
    );
    const stream = canvas.createPNGStream();
    stream.pipe(out);
    ...
    out.on("finish", () => {
      console.log(
        "Character Showcase generado y guardado como character-showcase.png"
      );
    });
  }
