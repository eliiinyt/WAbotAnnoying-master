...
const Init = async () => {
  try {
    ...
    client.on('message', async (msg) => {
      ...
      if (message.chat === '120363422219630188@g.us') {
        return;
      };
      if (!message.command && !message.args[0]) {
        return;
      }

      const command = commandLoader.getCommand(message.command === null ? message.args[0] : message.command);
      console.log(command);
      if (command?.name === null || command?.name === undefined) {
        return;
      }
      console.log(command?.isPrefix);
      if (command?.isPrefix === true || command?.isPrefix === null || command?.isPrefix === undefined) {
        if (!message.prefix) {
          return;
        }
        if (!command) {
      ...
      try {
        ...
        if (command?.react === true || message?.react === null) {
          await message.react('✅');
        }
  ...
  try {
    ...
    if (
      ...
      if (message && typeof message.download === 'function') {
        ...
        // console.log(buffer);
        console.log(`Archivo descargado (${type}), tamaño: ${buffer.buffer.length / 1024} KB`);
        multimedia = buffer;
      } else {
