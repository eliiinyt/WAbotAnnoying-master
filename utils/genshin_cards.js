...
const character_details = player;
  ...
  await drawWeapon(ctx, character_card, canvasWidth / 1.85, canvasHeight / 1.3);
    ...
    ctx.beginPath();
      ...
      ctx.font = `${canvasHeight * 0.024}px 'zh-cn'`;
      ctx.fillText(
        `${weaponStat.fightPropName.get("es")}: ${weaponStat.isPercent ? (weaponStat.value * 100).toFixed(1) : weaponStat.value.toFixed(0)}${
          weaponStat.isPercent ? "%" : ""
        }`,
        rectX + imgWidth + 5,
        textY + 5
      );
 ...
 async function drawTalents(ctx, character_card, x, y, arcRadiusPercentage = 0.025, imageSizePercentage = 0.05) {
  ...
  async function drawArtifacts(ctx, character_card, x, y) {
    ...
    for (const artifact of artifacts) {
      ...
      ctx.fillText(
        `${artifact.mainstat.fightPropName}: ${artifact.mainstat.value}${
          artifact.mainstat.isPercent ? "%" : ""
        }`,
        rectX + imgWidth + 20,
      ...
      artifact.substats.total.forEach((substat) => {
        ...
        ctx.fillText(
          `${substat.total.fightPropName}: ${substat.total.value}${
            substat.total.isPercent ? "%" : ""
          }`,
          rectX + imgWidth + 20,
          substatsY
        );
